...
3. Run `npm test` (skips the local Grok binary probe if Grok is not installed). Use `npm run test:e2e` only with an authenticated Grok CLI.
4. Add the local marketplace with `codex plugin marketplace add "$PWD"`.
5. Install with `codex plugin add grok-subagent@eeaker-grok`.
