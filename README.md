...
本仓库由 [Eeaker](https://github.com/Eeaker/grok-subagent) 维护。它从 [Walvez/grok-subagent](https://github.com/Walvez/grok-subagent)（MIT）起步，已经过大量 Windows 本地化、token offload 和安全边界改造。上游版权见 `LICENSE` 与 `THIRD_PARTY_NOTICES.md`。
...
本机：Windows；Grok Build `1.0.13`（`%USERPROFILE%\\.grok\\bin\\grok.exe`）；Codex Desktop `0.153.0-alpha.5`；默认模型 `grok-4.6`。安装用 `.\install.ps1`。
...
`worker_mode="full"`（或 `allow_shell=true`）允许更完整的工具/命令能力。Linux/macOS 叠加 custom OS sandbox（fail-closed）。原生 Windows **不会拒绝**，但只提供 `security_level=logical`：policy + worktree，没有内核沙箱。不要把它宣传成 hard isolation。

需要 Windows 上的 hard isolation，把 **Codex / Bridge / Grok 整体放到 WSL2/Linux**。
