...
def resolve_model(explicit: str | None = None) -> str:
    for candidate in (explicit, os.environ.get("GROK_SEARCH_MODEL"), os.environ.get("GROK_MODEL"), DEFAULT_MODEL):
...
def default_cache_root() -> Path:
    if os.name == "nt":
        ...
        if local:
            return Path(local) / "grok-subagent" / "search-runs"
...
def write_capped_text(path: Path, content: str, limit: int = MAX_RESULT_BYTES) -> bool:
    ...
    if truncated:
        ...
        while prefix:
            try:
                ...
                prefix = prefix[:-1]
        data = prefix + (marker if len(prefix) + len(marker) <= limit else b"")
    path.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
    temporary = path.with_name(f".{path.name}.{uuid.uuid4().hex}.tmp")
...
def find_grok() -> str:
    names = ["grok.exe", "grok"] if os.name == "nt" else ["grok"]
    candidates: list[Path] = []
    for env_key in ("GROK_BIN", "GROK_BINARY"):
        value = os.environ.get(env_key)
        if value:
            candidates.append(Path(value))
    for name in names:
        candidates.append(Path.home() / ".grok" / "bin" / name)
    ...
    raise BridgeError(
        "grok_not_found",
        "Grok Build was not found. Install it, then make sure `grok` is available or ~/.grok/bin/grok exists. On Windows the expected path is %USERPROFILE%\\.grok\\bin\\grok.exe.",
    )
...
def run_grok(args: argparse.Namespace) -> int:
    ...
    if answer:
        ...
        artifact_truncated = write_capped_text(result_path, answer.rstrip() + "\n")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_parser.add_argument("--model", help="Grok model id. Defaults to GROK_SEARCH_MODEL, GROK_MODEL, or grok-4.6")
